...
- [はじめかた](#はじめかた)
  - [1. PuniGen.bat をダブルクリック](#1-punigenbat-をダブルクリック)
  - [2. モデルを用意する](#2-モデルを用意する)
- [用語](#用語)
- [画面](#画面)
  - [生成のながれ](#生成のながれ)
  - [結果に対する操作](#結果に対する操作)
...
- 生成した絵を起点に、少しだけ変えた案を出したり、2 倍に拡大したりできます。生成履歴から後で遡ることもできます。失敗作はその場で片づけられます。
...
### 1. `PuniGen.bat` をダブルクリック

準備と起動をまとめて行います。黒い画面に、いま何をしているかが出ます。

```
 ____  _  _  __ _  __  ___  ____  __ _
(  _ \/ )( \(  ( \(  )/ __)(  __)(  (  ) __/) \/ (/    / )(( (_ \ ) _) /    /
(__)  \____/\_)__)(__)\___/(____)\_)__)

  Puni Uses No Intricacy.

[1/4] uv を確認...
[2/4] 実行環境を確認...
[3/4] 依存パッケージを確認...
[4/4] PuniGen を起動します。ブラウザが自動で開きます...
```

初回は `uv` の導入、専用の Python 3.12 の取得、`.venv` の作成、torch などのインストールを行うので数分かかります。管理者権限は不要で、システムの Python には触りません。入れるパッケージは `requirements.lock` で推移的依存まで固定してあるので、実行する時期や環境が違っても同じ構成になります。

初回だけ、起動の前に GPU の認識結果が出ます。

```
CUDA: True / NVIDIA GeForce RTX 4070
```

`CUDA: False` の場合は [こまったら](#こまったら) を見てください。やり直すときは `.venv` フォルダを消して、もう一度ダブルクリックします。

2 回目からは [2/4] と [3/4] が「準備済み」を確かめるだけになり、そのまま起動します（実測で 0.2 秒）。ブラウザが自動で開きます。終了するときは、ブラウザのタブではなく黒い画面を閉じてください。
...
モデルのメタデータは同じ場所に `モデル名.safetensors.json` として控えます。
...
履歴の画像を選ぶと、その絵の条件が生成情報欄に出るのに加えて、プレフィックス・プロンプト・ネガティブ・Steps・CFG・Sampler・Clip Skip・seed・サイズ・LoRA が左の入力欄へ戻ります。そのまま生成を押せば同じ絵が出ますし、一部だけ直して作り直すこともできます。

画像に記録されているのはプレフィックスとプロンプトを連結した 1 本の文字列なので、そのモデルのプリセットを見て 2 つの欄に分け直しています。プリセットを変えたなどで分けられないときは、プレフィックス欄を空にして全部をプロンプト欄へ入れます。連結した結果は変わらないので、どちらでも同じ絵が作れます。
...
ギャラリーで画像を選ぶと、4 つのボタンが使えます。

| ボタン | 動作 |
| --- | --- |
| 少しだけ変える | seed を保ったまま、少しだけ違う絵を 2 枚出します |
| 解像度を 2 倍 | 元の絵柄のまま拡大します（832x1216 → 1664x2432）。約 1.5 秒 |
| ゴミ箱 | `outputs/.trash/` へ移します。失敗作を履歴から片づけるためのものです |
| ★ | お気に入りの追加と解除。追加すると `favorites/` へ移動し、`outputs/` を整理しても残ります |

ゴミ箱のボタンは ★ の左隣にあります。押しても消えてはいません。`outputs/.trash/` へ移すだけなので、押し間違えたらフォルダから出せば履歴に戻ります。ゴミ箱の中身は自動では減らないので、要らなくなったらエクスプローラーでフォルダごと消してください。

お気に入りに入れた画像は削除できません。★ を押して解除してからにしてください。残すために選り分けた画像が、履歴を片づけるボタンで消えてしまわないようにしています。
...
`.venv` を消して `PuniGen.bat` を実行し直してください。NVIDIA ドライバが古い場合は更新が必要です（50 系は CUDA 12.8 対応ドライバが必須）。
...
依存が増えたバージョンに更新したときも、利用者側の操作は要りません。`PuniGen.bat` は起動のたびに `requirements.lock` と `.venv` を突き合わせるので、足りないパッケージは次回の起動で入ります（LoRA 対応では `peft` が増えました）。そろっているときは 18ms で終わり、ネットにも出ません（実測）。

なお compel だけは `PuniGen.bat` が `--no-deps` で個別に入れています。依存に notebook（Jupyter 一式）を宣言しているものの実行時には読まないためで、ロックに含めると無関係なパッケージが大量に増えます。
...
モデル（RealESRGAN_x4plus_anime_6B、約 17MB）は初回に一度だけ取得して `models/upscaler/` へ置きます。取得したものは SHA256 と照合してから置きます（16ms、実測）。中身が違えば置かずに知らせるので、壊れたファイルを読み込んで分かりにくいエラーになることはありません。モデル自体は 4 倍なので、4 倍にしてから目的の倍率へ縮めています。一度大きくしてから縮めた方が輪郭がなめらかになるためです。倍率は `upscaler.py` の `SCALE` で変えられます。
...
拡大したものには `_x2`、変分には `_var1` / `_var2` が付きます。お気に入りにした画像は `favorites/` へ、削除した画像は `outputs/.trash/` へ移動します。
