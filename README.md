...
- Civitai の検索とダウンロードを画面内で行えます。SDXL 系以外は結果に出さないので、動かないモデルを落とす心配がありません。
...
- 生成した絵を起点に、少しだけ変えた案を出したり、1.5 倍に高解像度化したりできます。生成履歴から後で遡ることもできます。
...
手元の `.safetensors` を使う場合は `models/checkpoints/` に置きます。Textual Inversion は `models/embeddings/` に置くと、ファイル名がそのままトリガーワードになります（SDXL 形式のみ）。
...
| お気に入り | 残したい絵を集めた場所です |
| モデルを追加 | Civitai を検索してダウンロードします |

生成タブの右側は「今回の結果」と「履歴」に分かれています。履歴には `outputs/` の画像が新しい順に並び、過去の絵を選んでそのまま作り直せます。
...
生成中は途中経過が流れます。気に入らなければ中止で打ち切れます。完成すると `outputs/` に自動保存されます。
...
| 解像度を 1.5 倍 | 拡大してから描き直し、細部を足します（832x1216 → 1248x1824） |
| ★ | `favorites/` へ移動します。`outputs/` を整理しても残ります |

★ ボタンは、選んでいる画像が既にお気に入りにある場合「★ 済」に変わって色が付き、押せなくなります。同じ画像を二重に入れる操作を防ぐためです。

ボタンは 1 組だけで、「今回の結果」と「履歴」のうち開いている側の画像に作用します。履歴の絵に対して実行すると、自動的に「今回の結果」へ切り替わって結果が出ます。

生成した画像には、プロンプトや seed が PNG に埋め込まれます。履歴から作り直せるのはこのためです。この機能より前に作った画像にはこの情報が無いので、お気に入りへの移動だけできます。
...
補完候補に絵師名は含めていません。特定の作家の画風を名指しで模倣する用途に直結するためです。手で入力することは妨げていません。
...
<details>
<summary>もっと細かい設定（プリセット / VRAM 判定 / 生成情報の読み方）</summary>
...
画面全体をブラウザのズーム 80% と同じ見え方にしています。Gradio の既定は余白も文字も大きめで、フル HD だとページ全体が 1070px になり縦スクロールが出るためです（実測）。80% にすると 985px に収まり、ちょうど 1 画面になります。
...
高解像度化したものには `_hires`、変分には `_var1` / `_var2` が付きます。お気に入りにした画像は `favorites/` へ移動します。
