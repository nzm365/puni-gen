...
| 削除 | `outputs/.trash/` へ移します。失敗作を履歴から片づけるためのものです |
| ★ | お気に入りの追加と解除。追加すると `favorites/` へ移動し、`outputs/` を整理しても残ります |

削除といっても消してはいません。`outputs/.trash/` へ移すだけなので、押し間違えたらフォルダから出せば履歴に戻ります。ゴミ箱の中身は自動では減らないので、要らなくなったらエクスプローラーでフォルダごと消してください。
