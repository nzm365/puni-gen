...
左が操作パネル、右が生成結果。モデルを選ぶとプリセットが自動で入るので、プロンプトを書いて **生成** を押すだけです。生成中は途中経過が流れ、気に入らなければ **中止** で打ち切れます。生成情報の 2 行目には工程別の実測時間が出ます。

主な機能:

- **モデルをその場で追加** — 「モデルを追加」タブから Civitai を検索して、サムネイルを選ぶだけでダウンロード（[2-1](#2-1-civitai-から追加する画面から)）
- **Danbooru タグの入力補完** — プロンプト欄で候補がポップアップ。オフラインで動作（[タグ補完](#プロンプトのタグ補完)）
- **Civitai と同じプロンプト記法** — `(tag:1.3)` の強調と 77 トークン超に対応。配布ページのプロンプトをそのまま貼れます（[プロンプトの書き方](#プロンプトの書き方civitai-と同じ記法が使えます)）
- **VRAM 事故の自動回避** — 空き容量を見て配置を決め、共有メモリ退避による激遅化を未然に防ぎます（[7. VRAM 判定](#7-vram-判定)）
...
- **77 トークン超**: CLIP の上限を超えるプロンプトは自動で分割して全部渡されます。後半に書いた画風タグが切り捨てられることはありません。
...
- 候補は色分けされます（一般 / 版権 / キャラ）。`models/embeddings/` に置いた embedding のトリガーワードも候補の先頭に出ます。
- **イラストレーター名タグは候補に出ません。** 特定の作家の画風を名指しで模倣する用途に直結するため、辞書の生成段階で除外しています・
