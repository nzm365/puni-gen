...
タブは 4 つあります。一覧は開くたびに自動で最新になります。

| タブ | 内容 |
| --- | --- |
| 生成 | プロンプトを書いて生成します。左が操作、右が結果です |
| 生成履歴 | `outputs/` の画像を一覧します。選んでそのまま作り直せます |
| お気に入り | 残したい絵を集めた場所です |
...
| ボタン | 動作 |
| --- | --- |
| この絵を少しだけ変える | seed を保ったまま、少しだけ違う絵を 2 枚出します |
| 1.5 倍に高解像度化 | 拡大してから描き直し、細部を足します（832x1216 → 1248x1824） |
| ★ お気に入り | `favorites/` へ移動します。`outputs/` を整理しても残ります |

同じ操作は生成履歴タブからもできます。過去の絵を選んで押すと、生成タブに移動して結果が出ます。
