...
**`SPEED_TWEAKS` について**
`engine.py` に速度優先の数値設定（cuDNN autotune / TF32 / channels_last / QKV 融合）がありますが、**既定は `False`** です。QKV 融合だけで重みが 1GB 以上増えるため、12GB クラスのカードでは VRAM があふれて逆に大幅に遅くなります（RTX 5070 で 0.9 秒/step → 51 秒/step を実測）。VRAM 16GB 以上で余裕がある場合にのみ `True` を試す価値があります。なお `True` にするとビット単位の再現性は保証されなくなります。
